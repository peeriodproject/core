	...
	private _setupListeners ():void {
		...
		this._protocolConnectionManager.on('hydraMessage', (identifier:string, ip:string, message:ReadableMessageInterface) => {
			...
			if (msgToEmit) {
				...
				logger.log('hydraReaction', 'Message Received on ' + identifier, {type: msgToEmit.getMessageType(), circuitId: msgToEmit.getCircuitId(), socketIdent: identifier, circuitNodes: JSON.stringify(this._circuitNodes)});
