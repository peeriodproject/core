...
describe('CORE --> TOPOLOGY --> ContactNodeAddress', function () {
	...
	var buffersMatch = function (a:Buffer, b:Buffer):boolean {
	...
	it('should correctly transform IPv6 to buffer', function () {
		var port = 55555,
			expected = new Buffer([0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x42, 0x83, 0x29, 0xd9, 0x03]);

		var ip1 = '2001:0db8:0000:0000:0000:ff00:0042:8329',
			c1 = new ContactNodeAddress(ip1, port);

		buffersMatch(c1.getAddressAsByteBuffer(), expected).should.be.true;

		var ip2 = '2001:db8::ff00:42:8329',
			c2 = new ContactNodeAddress(ip2, port);
		buffersMatch(c2.getAddressAsByteBuffer(), expected).should.be.true;

		var ip3 = '::1',
			c3 = new ContactNodeAddress(ip3, port);
