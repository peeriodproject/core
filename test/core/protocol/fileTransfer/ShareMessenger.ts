...
describe('CORE --> PROTOCOL --> FILE TRANSFER --> ShareMessenger', function () {
	...
	var circuitManager1:any = new events.EventEmitter();
	var circuitManager2:any = new events.EventEmitter();
	var circuitCount1:any = 0;
	var circuitCount2:any = 0;
	var transferMessageCenter1:any = new events.EventEmitter();
	var transferMessageCenter2:any = new events.EventEmitter();
	var middleware1:any = new events.EventEmitter();
	var middleware2:any = new events.EventEmitter();

	var messenger1:ShareMessenger = null;
	var messenger2:ShareMessenger = null;

	var sendMessage1 = function (remoteIdentifier:string, expectedNextIdentifier:string, callback:any) {
	...
	var sendMessage2 = function (remoteIdentifier:string, expectedNextIdentifier:string, callback:any) {
	...
	it('messenger1 should successfully initiate conversation as soon as there are circuits', function (done) {
		middleware1.once('msg', () => {
			middleware1.emit('res', true);
		});

		middleware2.once('msg', () => {
			middleware2.emit('res', true);
		});

		transferMessageCenter2.once('MESSAGE_023e6776d0afe3b05e216c6beab25deb', (circuitId, payload) => {
		...
		sendMessage1('023e6776d0afe3b05e216c6beab25deb', 'a5ee0ac65e3a2daac53867c544caa355', function (err, payload) {
			(<any> messenger1)._messageReceivedThroughCircuitId.should.equal('bar');
	...
	it('messenger1 and 2 should both timeout', function (done) {
		...
		sendMessage2('c61d48915194b4fcc243ee0e427c8a3c', 'b1e539cf3e653b5b1caa2c084bb758aa', (err, payload) => {
			err.message.should.equal('Maximum tries exhausted.');
			(transferMessageCenter2.lastCircuitIdUsed == null).should.be.true;
			checkAndDone();
		});

		middleware1.once('messenger', (err, payload) => {
			err.message.should.equal('Maximum tries exhausted.');
			(transferMessageCenter1.lastCircuitIdUsed == null).should.be.true;
			checkAndDone();
		});
	...
	it('messenger2 should send a last message', function (done) {
		...
		transferMessageCenter2.once('MESSAGE_29aaaf78b9f91eecee06794f9904a3c8', (circuitId, payload) => {
		...
		sendMessage1('29aaaf78b9f91eecee06794f9904a3c8', '22cd04277cfb462fffa763ffb4934763', function (err, payload) {
			...
			(<any>messenger1)._waitForResponseTimeout.should.equal(0);
			((<any>messenger1)._currentMessageListener == null).should.be.true;
	...
	before(function () {
		...
		configStub = testUtils.stubPublicApi(sandbox, ObjectConfig, {
			get: function (what) {
				if (what === 'fileTransfer.shareMessaging.maximumNumberOfMessageTries') return 1;
				if (what === 'fileTransfer.shareMessaging.waitForResponseMessageInSeconds') return 1;
			}
		});
		...
		transferMessageCenter1.issueExternalFeedToCircuit = function (nodesToFeedBlock, payloadToFeed, circuitId) {
			...
			if (circuitCount1 === 0) return false;

			this.lastCircuitIdUsed = circuitId;

			middleware1.once('res', (flag) => {
		...
		transferMessageCenter2.issueExternalFeedToCircuit = function (nodesToFeedBlock, payloadToFeed, circuitId) {
			if (circuitCount2 === 0) return false;

			this.lastCircuitIdUsed = circuitId;

			middleware2.once('res', (flag) => {
				if (flag) {
					setImmediate(function () {
