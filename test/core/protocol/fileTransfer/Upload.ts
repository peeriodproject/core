...
import ReadableFileTransferMessageInterface = require('../../../../src/core/protocol/fileTransfer/messages/interfaces/ReadableFileTransferMessageInterface');
import FeedingNodesMessageBlock = require('../../../../src/core/protocol/fileTransfer/messages/FeedingNodesMessageBlock');
import FeedingNodesBlockMaintainerFactory = require('../../../../src/core/protocol/fileTransfer/share/FeedingNodesBlockMaintainerFactory');
import FeedingNodesBlockMaintainer = require('../../../../src/core/protocol/fileTransfer/share/FeedingNodesBlockMaintainer');
import ShareMessengerFactory = require('../../../../src/core/protocol/fileTransfer/share/ShareMessengerFactory');
import ShareMessenger = require('../../../../src/core/protocol/fileTransfer/share/ShareMessenger');
import FileBlockWriterFactory = require('../../../../src/core/fs/FileBlockWriterFactory');
...
describe('CORE --> PROTOCOL --> FILE TRANSFER --> Upload (integration with Download) @current', function () {
	...
	var filepath:string = testUtils.getFixturePath('core/fileTransfer/snowden_brighton.jpg');
	...
	var createFeedingNodesBlockMaintainerFactory = function (forWhat) {
		...
		return testUtils.stubPublicApi(sandbox, FeedingNodesBlockMaintainerFactory, {
			create: function () {
				var maintainer:any = new events.EventEmitter();
			...
			createMessenger: function () {
				return testUtils.stubPublicApi(sandbox, ShareMessenger, {
					pipeLastMessage: function (payloadToFeed) {
						var msg:ReadableFileTransferMessageInterface = readableFileTransferMessageFactory.create(payloadToFeed);
	...
	it('should completely upload and download the file', function (done) {
		...
		var check = function () {
			if (downloadComplete && uploadComplete) done();
		}
		...
		theUpload.once('killed', function (reason) {
			...
			else throw new Error('Bad reason.');
		});
				...
				downloaderMiddleware.once('sending', function (type, ident, payload) {
					...
					uploaderMiddleware.once('sending', function () {
						downloaderMiddleware.once('sending', function () {
							...
							(<any> theUpload)._fileReader._fileDescriptor = -1;
	...
	before(function () {
		...
		transferMessageCenterStub = testUtils.stubPublicApi(sandbox, TransferMessageCenter, {
			wrapTransferMessage (messageType, transferId, payload) {
		...
		configStub = testUtils.stubPublicApi(sandbox, ObjectConfig, {
			get: function (what) {
				if (what === 'fileTransfer.uploadBlockSizeInBytes') return 204800;
			}
		});
	});
