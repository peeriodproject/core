/// <reference path='../../../test.d.ts' />

require('should');

import ReadableShareRatifyMessage = require('../../../../src/core/protocol/fileTransfer/share/messages/ReadableShareRatifyMessage');
import FeedingNodesMessageBlock = require('../../../../src/core/protocol/fileTransfer/messages/FeedingNodesMessageBlock');

describe('CORE --> PROTOCOL --> FILE TRANSFER --> ReadableShareRatifyMessage', function () {

	it('should correctly deformat the message', function () {

		var feedingNodesBlock:Buffer = FeedingNodesMessageBlock.constructBlock([{ip: '1.1.1.1', port: 80, feedingIdentifier: 'cafebabecafebabecafebabecafebabe'}]);
		var filename:string = 'This is a file.jpg';
		var filesize:number = 12345632434;
		var dhPayload:string = '';
		var hash:string = '770606aff59bca0b045d8e0eaf5a7a7568bc1d39';

		var filesizeBuffer = new Buffer('00bc6120000001b2', 'hex');

		var fullBuffer = Buffer.concat([new Buffer(dhPayload, 'hex'), new Buffer(hash, 'hex'), feedingNodesBlock, filesizeBuffer, new Buffer(filename, 'utf8')]);

		var msg = new ReadableShareRatifyMessage(fullBuffer);

		msg.getDHPayload().toString('hex').should.equal(dhPayload);
		msg.getSecretHash().toString('hex').should.equal(hash);

		msg.deformatDecryptedPart(msg.getEncryptedPart());

		msg.getDeformattedDecryptedFeedingNodesBlock().toString('hex').should.equal(feedingNodesBlock.toString('hex'));
		msg.getDeformattedDecryptedFilename().should.equal(filename);
		msg.getDeformattedDecryptedFileSize().should.equal(filesize);

	});

	it('should throw the appropriate errors', function () {
		(function () {
			new ReadableShareRatifyMessage(new Buffer(255));
		}).should.throw('ReadableShareRatifyMessage: Diffie-Hellman bad length! Expected 256 bytes.');

		(function () {
			new ReadableShareRatifyMessage(new Buffer(258));
		}).should.throw('ReadableShareRatifyMessage: Secret hash bad length! Expected 20 bytes (SHA-1)');
	});

});