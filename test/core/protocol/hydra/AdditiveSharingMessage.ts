...
describe('CORE --> PROTOCOL --> HYDRA --> AdditiveSharingMessage', function () {
	...
	it('should correctly deformat a message with ipv4', function () {
		// 44.123.255.7:55555
		var ipv4Address = new Buffer([0x04, 44, 123, 255, 7, 0xd9, 0x03]),
			payload = new Buffer('foobario', 'utf8');
	...
	it('should correctly deformat a message with ipv6', function () {
		...
		var ipv6Address = new Buffer([0x06, 0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x42, 0x83, 0x29, 0xd9, 0x03]),
			payload = new Buffer('muschi');
	...
	it('should throw an error when deformatting a message with unknow indicator byte', function () {
		var ipv4Address = new Buffer([0x07, 44, 123, 255, 7, 0xd9, 0x03]),
			payload = new Buffer('foobario', 'utf8');
