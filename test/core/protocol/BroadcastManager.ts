...
describe('CORE --> PROTOCOL --> BROADCAST --> BroadcastManager @current', function () {
	...
	// CHECKERS
	var writtenMessage:Buffer = null;
	var writtenTo = [];
	...
	var readableFactoryStub:any = null;
	...
	before(function () {
		...
		configStub = testUtils.stubPublicApi(sandbox, ObjectConfig, {
			get: function (what) {
				if (what === 'topology.bitLength') return 8;
				if (what === 'topology.alpha') return 2;
				if (what === 'protocol.broadcast.broadcastLifetimeInSeconds') return 1;
			}
		});
