...
describe('CORE --> PROTOCOL --> FIND CLOSEST NODES --> FindClosestNodesCycle', function () {
	...
	var searchForHex=	'6d';
	var startWith	=[	'01100000',
						'00100100'];

	var responder1	=	'00100100';
	var response1	=[	'01101101',
						'01100000',
						'10101101',
						'11011011',
						'01111111'];
	...
	var emitReply = function (identifier, fromBitString, array) {
		...
		else throw new Error('no listener bound.')
	}
	...
	it('should probe the next alpha nodes with loose parallelism', function (done) {
		setTimeout(function () {
			if (findCycle.getProbeList().length === 1) {
				setTimeout(function () {
					if (findCycle.getProbeList().length === 0 && findCycle.getRegisteredIdentifiers().length === 6) done();
				}, 200);
			}
		}, 200);
	});
