 ...
 */
class InflatingFileBlockWriter implements FileBlockWriterInterface {
	...
	public writeBlock (byteBlock:Buffer, callback:(err:Error, fullCountOfWrittenBytes?:number, isFinished?:boolean) => any):void {
		...
		zlib.inflateRaw(byteBlock, (err:Error, inflatedByteBlock:Buffer) => {
			if (err) {
				callback(err);
			}
			else {
				var expectedBytesToWrite:number = inflatedByteBlock.length;
...
export = InflatingFileBlockWriter;